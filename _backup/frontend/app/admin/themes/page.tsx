        ...
        <div className="p-6 border-b border-white/10 sticky top-0 bg-[#171717]/80 backdrop-blur-md z-10">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-black text-white">Конструктор тем</h1>
          ...
          <div className="mt-4 p-3 rounded-xl bg-white/5 border border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              {globalEnabled ? <Globe size={18} className="text-emerald-400" /> : <Globe size={18} className="text-white/40" />}
              <div>
            ...
            <button
              ...
              className={`relative w-12 h-6 rounded-full transition-colors ${
                globalEnabled ? "bg-emerald-500" : "bg-white/20"
              }`}
            >
              <span
...
function ThemeCard({
  ...
  return (
    <div className={`rounded-2xl overflow-hidden border transition-all ${
      isCurrent ? "border-[#8b5cf6] ring-2 ring-[#8b5cf6]/50" : "border-white/10"
    } bg-[#1f1f23]`}>
      {/* Превью */}
      <div className="relative h-40 overflow-hidden group">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-3 gap-2">
          <button
            onClick={onPreview}
            className="p-2 rounded-lg bg-white/20 backdrop-blur text-white hover:bg-white/30 transition-all"
      ...
      <div className="p-3">
        <div className="flex items-center justify-between gap-2 mb-2">
          <h3 className="font-bold text-white truncate">{theme.name || "Без названия"}</h3>
          <span className="text-[10px] uppercase font-black px-1.5 py-0.5 rounded bg-white/10 text-white/60 shrink-0">
            {theme.type}
          </span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-1 text-[10px] text-white/40 mb-3">
          <div className="flex flex-col items-center p-1 rounded bg-white/5">
            <Zap size={10} />
        ...
        <button
          ...
          className={`w-full py-2 rounded-lg text-xs font-bold transition-all ${
            theme.is_default
              ...
              : "bg-white/5 text-white/70 hover:bg-[#8b5cf6]/20 hover:text-[#8b5cf6]"
          }`}
        >
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[301] flex items-center justify-center p-4 pointer-events-none">
        <div className="w-full max-w-2xl bg-[#1f1f23] border border-white/15 rounded-2xl shadow-2xl pointer-events-auto max-h-[90vh] overflow-y-auto">
          <div className="sticky top-0 bg-[#1f1f23] p-4 border-b border-white/10 flex items-center justify-between z-10">
          ...
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg border border-white/15 bg-white/5 text-white placeholder-white/40 focus:outline-none focus:border-[#8b5cf6]"
              />
            </div>

            {/* Тип анимации */}
            <div>
              <label className="block text-xs font-bold text-white/60 mb-1.5">Тип анимации</label>
                ...
                {theme.colors.map((c, i) => (
                  <div key={i} className="relative group">
                    <input
                      ...
                      className="w-full h-12 rounded-lg cursor-pointer border border-white/20 bg-transparent"
                    />
                    <div className="text-[10px] text-white/40 text-center mt-0.5 font-mono">
                      {c}
                    </div>
            ...
            <div className="p-3 rounded-xl bg-white/5 border border-white/10 flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-xs font-bold text-white/80">Уровень доступа</p>
              ...
              <input
                ...
                className="w-16 px-2 py-1 rounded border border-white/15 bg-white/5 text-white text-sm text-center focus:outline-none focus:border-[#8b5cf6]"
              />
            </div>
          </div>
          ...
          <div className="sticky bottom-0 bg-[#1f1f23] p-4 border-t border-white/10 flex gap-2">
            <button
              onClick={onClose}
              className="flex-1 py-2.5 rounded-lg border border-white/15 text-white/80 font-bold hover:bg-white/5"
            >
              Отмена
            </button>
            <button
