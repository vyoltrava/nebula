...
export function ChatsSection({ me }: { me: any }) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 h-[calc(100vh-200px)]">
      {/* Список чатов */}
      <div className={`w-full md:w-96 border border-white/10 rounded-xl bg-white/5 flex flex-col ${activeChat ? "hidden md:flex" : "flex"}`}>
        <div className="p-3 border-b border-white/10">
          ...
          <p className="text-xs text-white/40 mt-2">Всего: {chats.length} • Показано: {filtered.length}</p>
        </div>
        <div className="flex-1 overflow-y-auto">
          {filtered.map((c) => (
            <button key={c.id} onClick={() => openChat(c)}
              ...
              <div className="min-w-0 flex-1">
                <p className="font-bold text-white text-sm truncate">{c.name || (c.is_group ? "Группа" : "Диалог")}</p>
          ...
          {filtered.length === 0 && <p className="text-center text-white/40 text-sm py-8">Чатов не найдено</p>}
        </div>
      </div>

      {/* Сообщения */}
      <div className={`flex-1 border border-white/10 rounded-xl bg-white/5 flex flex-col ${activeChat ? "flex" : "hidden md:flex"}`}>
        {!activeChat ? (
          ...
          <>
            <div className="p-3 border-b border-white/10 flex items-center gap-3">
              <button onClick={() => setActiveChat(null)} className="p-2 rounded-lg text-white/60 hover:text-white hover:bg-white/10 md:hidden">
                <ArrowLeft size={18} />
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              {!loading && messages.length === 0 && <p className="text-center text-white/40 text-sm">Сообщений нет</p>}
              {!loading && messages.map((m) => (
                <div key={m.id} className="flex items-start gap-2.5">
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button onClick={() => togglePin(m)} className="p-1.5 rounded-lg text-white/40 hover:text-[#8b5cf6] hover:bg-white/10" title={m.pinned ? "Открепить" : "Закрепить"}>
                      {m.pinned ? <PinOff size={14} /> : <Pin size={14} />}
                    </button>
                    {me.is_admin && (
                      <button onClick={() => deleteMsg(m.id)} className="p-1.5 rounded-lg text-white/40 hover:text-red-400 hover:bg-red-500/10" title="Удалить">
                        <Trash2 size={14} />
                      </button>
                    )}
                  </div>
