            ...
            <div className="max-h-60 overflow-y-auto p-2 space-y-1">
              {accounts.map((acc) => (
                <div key={acc.userId} className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 transition-colors group">
                  ...
                  {accounts.length > 1 && (
                    <button onClick={() => { removeAccount(acc.userId); onClose?.(); }} className="p-1.5 text-white/30 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
                      <X size={14} />
                    </button>
                  )}
                </div>
