            ...
            <div key={pack.id} className={`border rounded-2xl p-4 ${pack.is_active ? "border-white/15 bg-white/5" : "border-white/5 bg-white/[0.02] opacity-60"}`}>
              <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
                ...
                <div className="flex gap-1.5">
                  ...
                  <button onClick={() => { setEditingPack({ ...pack }); setSelectedEmojis([]); setPendingFiles([]); setPendingFileUrls([]); setShowEditor(true); }} className="p-1.5 rounded-lg text-white/50 hover:text-white hover:bg-white/10"><Edit3 size={14} /></button>
          ...
          <div className="fixed inset-0 z-[301] flex items-center justify-center p-4 pointer-events-none overflow-y-auto">
            <div className="w-full max-w-2xl bg-[#1f1f23] border border-white/15 rounded-2xl shadow-2xl p-5 pointer-events-auto my-8">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => !savingPack && setShowEditor(false)} className="text-white/60 hover:text-white p-1"><X size={18} /></button>
              </div>
              <div className="space-y-4">
              ...
              <div className="flex gap-2 mt-5">
                <button onClick={() => !savingPack && setShowEditor(false)} disabled={savingPack} className="flex-1 py-2.5 rounded-lg border border-white/15 text-white/80 font-bold hover:bg-white/5 disabled:opacity-50">Отмена</button>
                <button onClick={savePack} disabled={savingPack || !editingPack.name?.trim()} className="flex-1 py-2.5 rounded-lg bg-yellow-500 text-black font-bold hover:bg-yellow-400 disabled:opacity-50">
          ...
          <div className="fixed inset-0 z-[301] flex items-center justify-center p-4 pointer-events-none overflow-y-auto">
            <div className="w-full max-w-md bg-[#1f1f23] border border-white/15 rounded-2xl shadow-2xl p-5 pointer-events-auto my-8">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => !savingBadge && setShowBadgeEditor(false)} className="text-white/60 hover:text-white p-1"><X size={18} /></button>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input type="color" value={editingBadge.glow_color || "#8b5cf6"} onChange={(e) => setEditingBadge({ ...editingBadge, glow_color: e.target.value })} className="w-8 h-8 rounded cursor-pointer bg-transparent border-0" />
                ...
                <div className="border-t border-white/10 pt-3 space-y-3">
                  <div>
                    <label className="block text-xs font-bold text-white/60 mb-1.5">Авто-выдача для роли (опционально)</label>
                    ...
                    <p className="text-[10px] text-white/30 mt-1">Если выбрать роль, значок появится у всех пользователей с этой ролью автоматически.</p>
                  </div>
              ...
              <div className="flex gap-2 mt-5">
                <button onClick={() => !savingBadge && setShowBadgeEditor(false)} disabled={savingBadge} className="flex-1 py-2.5 rounded-lg border border-white/15 text-white/80 font-bold hover:bg-white/5 disabled:opacity-50">Отмена</button>
                <button onClick={saveBadge} disabled={savingBadge || !editingBadge.name?.trim() || (!editingBadge.id && !badgeFile)} className="flex-1 py-2.5 rounded-lg bg-purple-500 text-white font-bold hover:bg-purple-400 disabled:opacity-50">
                  {savingBadge ? <Loader2 size={16} className="animate-spin mx-auto" /> : (editingBadge.id ? "Сохранить" : "Создать значок")}
                </button>
              </div>
            </div>
          </div>
