        ...
        <div className={`w-full md:w-80 lg:w-96 border-r border-white/10 flex flex-col bg-white/[0.02] ${activeId ? "hidden md:flex" : "flex"}`}>
          <div className="p-4 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-white text-lg">{t("support.myTickets")}</h2>
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {tickets.length === 0 && !showCreate && (
              <div className="text-center py-12 px-4">
                ...
                <p className="text-white/40 text-sm">{t("support.empty")}</p>
                <p className="text-white/25 text-xs mt-1">{t("support.emptyHint")}</p>
              </div>
            )}
            {tickets.map(ticket => (
              <button key={ticket.id} onClick={() => setActiveId(ticket.id)}
                className={`w-full text-left p-3 border-b border-white/5 transition-colors ${
                  activeId === ticket.id ? "bg-[#8b5cf6]/10 border-l-2 border-l-[#8b5cf6]" : "hover:bg-white/5 border-l-2 border-l-transparent"
                }`}>
                <div className="flex items-center justify-between mb-1">
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                    ticket.status === "open" ? "bg-green-500/20 text-green-400" : "bg-white/10 text-white/40"
                  }`}>
          ...
          {!activeId ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center max-w-xs">
                ...
                <h3 className="text-white/50 font-medium mb-2">{t("support.pick")}</h3>
                <p className="text-white/30 text-sm">{t("support.pickHint")}</p>
              </div>
            </div>
          ) : (
            <>
              <div className="p-3 border-b border-white/10 flex items-center gap-3 bg-white/[0.02]">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-white text-sm">{t("support.ticketN", { id: activeId })}</h3>
