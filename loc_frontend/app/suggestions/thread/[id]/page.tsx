...
const STATUS_CONFIG: Record<string, any> = {
  ...
  implemented: { color: "bg-green-500/20 text-green-400 border-green-500/30", icon: Zap },
...
export default function ThreadPage() {
  ...
  if (loading) return <div className="min-h-screen bg-[#171717] flex items-center justify-center"><p className="text-white/50 animate-pulse">{t("common.loading")}</p></div>;
  if (!thread) return <div className="min-h-screen bg-[#171717] flex items-center justify-center"><p className="text-white/50">{t("suggestions.notFound")}</p></div>;
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="border border-white/10 rounded-2xl p-6 bg-white/5 mb-6">
          <div className="flex items-center gap-2 flex-wrap mb-3">
            {thread.is_pinned && <Pin size={18} className="text-yellow-400" />}
            {thread.is_closed && (
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-white mb-4">{thread.title}</h1>
          ...
          <div className="flex items-center gap-3 mb-6 pb-6 border-b border-white/10">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-white/40">{t("suggestions.author")} · {fmtDate(thread.created_at)}</p>
            </div>
          ...
          <div className="border border-[#8b5cf6]/30 rounded-2xl p-4 bg-[#8b5cf6]/5 mb-6">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              <button onClick={() => setEditModal(true)}
                className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-white/70 text-xs font-bold hover:text-white flex items-center gap-1.5">
              ...
              <button onClick={() => api("PATCH", `/api/suggestions/thread/${threadId}/pin`, patchForm({ is_pinned: String(!thread.is_pinned) }))}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold border flex items-center gap-1.5 ${thread.is_pinned ? "bg-yellow-500/20 border-yellow-500/30 text-yellow-400" : "bg-white/5 border-white/10 text-white/70 hover:text-white"}`}>
              ...
              <button onClick={() => api("PATCH", `/api/suggestions/thread/${threadId}/close`, patchForm({ closed: String(!thread.is_closed) }))}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold border flex items-center gap-1.5 ${thread.is_closed ? "bg-green-500/20 border-green-500/30 text-green-400" : "bg-red-500/20 border-red-500/30 text-red-400"}`}>
              ...
              <button onClick={deleteThread}
                className="px-3 py-1.5 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 text-xs font-bold flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              <select value={thread.status}
                ...
                className="px-2 py-2 rounded-lg bg-[#171717] border border-white/10 text-white text-xs outline-none">
        ...
        {/* Комментарии */}
        <div className="mb-6">
          <h2 className="text-xl font-black text-white mb-4">{t("suggestions.commentsTitle")} ({comments.length})</h2>
          <div className="space-y-4">
            {comments.map((c) => (
              <div key={c.id} className="border border-white/10 rounded-xl p-5 bg-white/5">
                <div className="flex items-start gap-3">
                  <Avatar src={c.author?.avatar_url} name={c.author?.display_name} id={c.author?.id} size={36} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-white font-bold">{c.author?.display_name}</span>
                      <span className="text-xs text-white/40">{fmtDate(c.created_at)}</span>
                      {(canManage || me?.id === c.author?.id) && (
                        <button onClick={() => deleteComment(c.id)} className="ml-auto p-1.5 rounded text-red-400/60 hover:text-red-400 hover:bg-red-500/10">
                          <Trash2 size={14} />
                        </button>
                      )}
                    </div>
            ...
            {hasMore && (
              <button onClick={() => loadThread(cursor)} className="w-full py-3 rounded-xl border border-white/10 bg-white/5 text-white/60 hover:bg-white/10 hover:text-white">
                {t("common.loadMore")}
              </button>
            )}
            {comments.length === 0 && (
              <div className="text-center py-12 border border-white/10 rounded-xl bg-white/5">
                <p className="text-white/50">{t("suggestions.noComments")}</p>
                <p className="text-white/30 text-sm mt-1">{t("suggestions.beFirstComment")}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {canWrite ? (
          ...
          <div className="px-4 py-4 rounded-xl border border-red-500/30 bg-red-500/10 text-red-400 text-sm flex items-center gap-2">
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[300] flex items-center justify-center p-4">
      <div className="w-full max-w-2xl bg-[#1f1f23] border border-white/15 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-black text-white">{t("suggestions.editThread")}</h3>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onClose} className="flex-1 py-3 rounded-lg border border-white/15 text-white/80 font-bold hover:bg-white/5">{t("common.cancel")}</button>
        </div>
      </div>
    </div>
  );
