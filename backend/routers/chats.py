...
from dependencies import *  # noqa: F401,F403  (модели, хелперы и классы запросов из main)
...
def serialize_chat_for_user(chat: Chat, user_id: int, session: Session) -> dict:
    ...
    if chat.is_group:
        return {
            ...
            "is_group": True,
            "is_secret": False,
            "is_prism": getattr(chat, 'is_prism', False),
...
@router.get("/api/chat-drafts/{chat_id}")
def get_chat_draft(
    chat_id: str,
    user: User = Depends(get_current_user),
    session: Session = Depends(get_session),
...
@router.put("/api/chat-drafts/{chat_id}")
def put_chat_draft(
    chat_id: str,
    body: ChatDraftIn,
    user: User = Depends(get_current_user),
...
@router.delete("/api/chat-drafts/{chat_id}")
...
def list_chat_folders(
    ...
    folders — кастомные папки юзера.
    chats — чаты (ЛС, обычные группы), не привязанные к папкам.
    """
    memberships = session.exec(
        select(ChatMember.chat_id).where(ChatMember.user_id == user.id)
    ).all()
...
@router.patch("/api/chats/folders/{folder_id}")
...
@router.delete("/api/chats/folders/{folder_id}")
...
@router.post("/api/chats/folders/{folder_id}/chats")
...
@router.delete("/api/chats/folders/{folder_id}/chats/{chat_id}")
...
@router.patch("/api/chats/{chat_id}/members/{user_id}/role")
...
@router.get("/api/chats/{chat_id}/mute")
...
@router.patch("/api/chats/{chat_id}/mute")
...
@router.get("/api/chats/{chat_id}/invites")
...
@router.post("/api/chats/{chat_id}/invites")
...
@router.delete("/api/chats/{chat_id}/invites/{invite_id}")
...
@router.post("/api/chats/{chat_id}/invite")
...
@router.delete("/api/chats/{chat_id}/invite")
