...
# Идемпотентно: ошибка глотается ВНУТРИ plpgsql (DO $$ EXCEPTION) — иначе
# провалившийся ALTER отравляет транзакцию и ВСЁ последующее падает с
# InFailedSqlTransaction (try/except в Python транзакцию не спасает!).
PG_IDENTITY_FIXES = [
...
def _safe_pg_ddl(ddl: str) -> str:
    """Обёртка: выполняет DDL, глотая ошибку на уровне БД (не отравляя транзакцию)."""
    return (
        "DO $$ BEGIN\n"
        f"  {ddl};\n"
...
def upgrade() -> None:
    ...
    if is_pg:
        ...
        # Ошибка глотается на уровне SQL (DO $$ EXCEPTION) — транзакция жива.
        for ddl in PG_IDENTITY_FIXES:
            op.execute(text(_safe_pg_ddl(ddl)))
