"""Initial schema bootstrap — перенесённый runtime DDL из main.py (идемпотентный).
...
def upgrade():
    # 0. Таблицы из SQLModel-моделей (идемпотентно, checkfirst=True)
    SQLModel.metadata.create_all(op.get_bind())
    # 1. Legacy-правки схемы (все запросы идемпотентны — IF NOT EXISTS / DO $$)
    _legacy_ddl()

def _legacy_ddl():
    """Исходный DDL-блок, выполнявшийся при старте приложения."""
    # ============================================================
