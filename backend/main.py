...
def get_rules(session: Session = Depends(get_session)):
    ...
    rules_obj = session.exec(select(SiteRules).order_by(SiteRules.id.desc()).limit(1)).first()
    
    if rules_obj:
        try:
            return json.loads(rules_obj.content)
        except:
            pass
    ...
    return {
        ...
        "subtitle": "Соблюдение правил обязательно для всех пользователей. Нарушение правил может привести к ограничениям или блокировке аккаунта.",
        "sections": [
            {
                "id": "general",
                "heading": "1. Общие положения",
                "items": [
                    "Используя NEBULA, вы соглашаетесь с данными правилами.",
                    "Администрация оставляет за собой право изменять правила без предварительного уведомления.",
                    "Незнание правил не освобождает от ответственности.",
                    "Администрация может применять меры по своему усмотрению в зависимости от тяжести нарушения.",
                ]
            },
            {
                "id": "respect",
                "heading": "2. Уважение к другим пользователям",
                "items": [
                    "Запрещены оскорбления, угрозы, дискриминация по любому признаку.",
                    "Запрещены преследование, буллинг, harassment.",
                    "Запрещена публикация личной информации других людей без их согласия (доксинг).",
                    "Запрещены призывы к насилию или ненависти.",
                ]
            },
            {
                "id": "content",
                "heading": "3. Контент",
                "items": [
                    "Запрещён спам, накрутка лайков/подписчиков, мошенничество.",
                    "Запрещён порнографический контент.",
                    "Запрещён контент с насилием, жестокостью, пытками.",
                    "Запрещена пропаганда наркотиков, экстремизма, терроризма.",
                    "Запрещена публикация материалов, нарушающих авторские права.",
                    "Запрещён контент, нарушающий законодательство вашей страны.",
                ]
            },
            {
                "id": "accounts",
                "heading": "4. Аккаунты",
                "items": [
                    "Запрещены фейковые аккаунты, имперсонация других людей.",
                    "Запрещена продажа/передача аккаунтов.",
                    "Запрещено использование ботов для автоматизации действий.",
                    "Один человек — один аккаунт (мультиаккаунты запрещены).",
                ]
            },
            {
                "id": "privacy",
            ...
            {
                "id": "punishments",
                "heading": "6. Меры наказания",
                "table": [
                    {
                        "num": "1",
                        "measure": "Предупреждение",
                        "description": "Выносится за первое незначительное нарушение. Действует 30 дней.",
                        "violations": "Мелкий спам, флуд, единичное оскорбление."
                    },
                    {
                        "num": "2",
                        "measure": "Удаление контента",
                        "description": "Пост/комментарий удаляется. Пользователь получает уведомление.",
                        "violations": "Нарушение правил контента, спам."
                    },
                    {
                        "num": "3",
                        "measure": "Временная блокировка",
                        "description": "Аккаунт блокируется на срок от 1 дня до 30 дней.",
                        "violations": "Повторные нарушения, оскорбления, спам."
                    },
                    {
                        "num": "4",
                        "measure": "Перманентная блокировка",
                        "description": "Аккаунт блокируется навсегда без возможности восстановления.",
                        "violations": "Систематические нарушения, угрозы, мошенничество."
                    }
                ],
                "note": "Администрация может комбинировать меры наказания или применять другие меры по своему усмотрению."
            }
        ],
        "footer": "Администрация NEBULA оставляет за собой право удалять любой контент и блокировать аккаунты без объяснения причин в случае грубых нарушений."
    }
...
def update_rules(
    ...
    # Сохраняем в БД
    rules_obj = SiteRules(content=content, updated_by=user.id)
    session.add(rules_obj)
    session.commit()
    return {"ok": True}
