...
# ============================================================
# 🎧 ЭНДПОИНТЫ ДЛЯ ФРОНТЕНДА ПОДДЕРЖКИ
# ============================================================
...
def list_support_tickets_staff(
    ...
    """Список тикетов для саппортеров (фронтенд админки)"""
    ...
    for t in tickets:
        ...
        result.append({
            ...
            "last_message": {
                "text": (last_msg.text or "📎 Вложение")[:60] if last_msg else None,
...
def get_ticket_messages(
    ...
    """Сообщения тикета — для саппортера ИЛИ автора тикета"""
    ...
    messages = session.exec(
        select(Message)
        .where(Message.chat_id == ticket.chat_id)
    ...
    return [{
        "id": m.id,
        "sender": user_out(senders.get(m.sender_id), session) if senders.get(m.sender_id) else None,
        "text": m.text,
        "media_url": m.media_url,
        "media_type": m.media_type,
        "created_at": m.created_at.isoformat(),
...
async def send_ticket_message(
    ...
    """Отправить сообщение в тикет (саппортер или автор)"""
    ticket = session.get(SupportTicket, ticket_id)
    if not ticket:
        raise HTTPException(404, "Тикет не найден")
    ...
    member = session.exec(
        select(ChatMember).where(
            ChatMember.chat_id == ticket.chat_id,
            ChatMember.user_id == user.id,
        )
    ).first()
    if not member:
    ...
    # Обновляем updated_at тикета (для сортировки)
    ticket.updated_at = datetime.now(timezone.utc)
    session.add(ticket)

    # Уведомления другим участникам
    other_members = session.exec(
        select(ChatMember).where(
            ChatMember.chat_id == ticket.chat_id,
            ChatMember.user_id != user.id,
    ...
    # WebSocket рассылка всем участникам чата
    all_member_ids = session.exec(
        select(ChatMember.user_id).where(ChatMember.chat_id == ticket.chat_id)
    ).all()
    await manager.broadcast_to_users(
...
def close_ticket_user_or_staff(
    ticket_id: int,
    user: User = Depends(get_current_user),
    session: Session = Depends(get_session),
):
    """Закрыть тикет — может саппортер ИЛИ автор тикета"""
    ticket = session.get(SupportTicket, ticket_id)
    if not ticket:
        raise HTTPException(404, "Тикет не найден")
