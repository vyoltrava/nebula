  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 py-4 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] flex items-center justify-center gap-4 sm:gap-6">
        ...
        {(status === 'ringing' || status === 'initiating') && isCaller && (
        ...
        {/* 🔴 СБРОС: доступен в ЛЮБОМ другом статусе (initiating / ended / unexpected),
            чтобы звонок никогда не висел без возможности сбросить */}
        {!['ringing', 'active', 'connecting'].includes(status) && (
          <button
            onClick={endCall}
