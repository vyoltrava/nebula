...
export default function CallModal() {
  ...
  // Медиа стартует muted, а после первого тапа пользователя размонтируется (unmute).
  ...
  useEffect(() => {
    if (remoteVideoRef.current && remoteStream) {
      ...
      remoteVideoRef.current.muted = !remoteUnmuted;
    }
    if (remoteAudioRef.current && remoteStream) {
      remoteAudioRef.current.srcObject = remoteStream;
      remoteAudioRef.current.muted = !remoteUnmuted;
    }
  }, [remoteStream, remoteUnmuted]);
  ...
  return (
    ...
    >
      {/* Удалённый собеседник — вписан в экран (object-contain), не растянут на весь экран */}
      ...
      {/* Аудио элемент — muted для iOS autoplay, unmuted после первого тапа */}
      {!isVideoCall && <audio ref={remoteAudioRef} autoPlay playsInline muted={!remoteUnmuted} />}

      {/* Свой (локальный) превью — всегда muted, чтобы не было обратной связи */}
      {isVideoCall && localStream && !isVideoOff && (
        <div className="absolute top-4 right-4 z-20">
          <video
            ...
            muted
            playsInline
            className="w-28 h-40 rounded-xl object-cover shadow-lg ring-2 ring-white/30"
          />
        </div>
      )}
