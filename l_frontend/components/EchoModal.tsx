...
export function EchoModal({ postId, onClose }: { postId: number; onClose: () => void }) {
    ...
    return (
      <div key={node.id} className="flex flex-col items-center">
        ...
        <div className={`w-full max-w-lg rounded-2xl border transition-all duration-300 ${
          isRoot 
            ? "bg-purple-500/[0.07] dark:bg-gradient-to-br dark:from-purple-900/30 dark:to-indigo-900/30 border-purple-400 dark:border-purple-500/50 shadow-[0_0_30px_rgba(168,85,247,0.12)] dark:shadow-[0_0_30px_rgba(168,85,247,0.2)]" 
            : "bg-gray-50 dark:bg-white/[0.04] border-line dark:border-white/15 hover:border-purple-400/40 hover:bg-gray-100 dark:hover:bg-white/[0.06]"
        }`}>
          <div className="p-4">
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 text-sm flex-wrap">
                  ...
                  <span className="text-gray-400 dark:text-white/25 text-xs">· {timeAgo(node.created_at)}</span>
                </div>
  ...
  return (
    ...
    >
      <div 
        className="w-full max-w-3xl max-h-[85vh] bg-ivory dark:bg-[#0d0d14] border border-purple-500/20 rounded-3xl shadow-[0_0_60px_rgba(168,85,247,0.15)] flex flex-col overflow-hidden relative animate-in zoom-in-95 duration-300" 
        onClick={e => e.stopPropagation()}
      >
        {/* Шапка */}
        <div className="relative flex items-center justify-between p-5 border-b border-line dark:border-white/10 bg-purple-500/[0.05] dark:bg-gradient-to-r dark:from-purple-900/20 dark:via-indigo-900/10 dark:to-transparent">
        ...
        <div className="relative flex-1 overflow-y-auto p-5 scrollbar-thin scrollbar-thumb-purple-500/30 scrollbar-track-transparent">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-gray-500 dark:text-white/50 text-sm text-center max-w-xs">
                {t("echo.empty")}
              </p>
            </div>
          ) : (
