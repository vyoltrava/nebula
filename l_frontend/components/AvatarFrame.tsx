  ...
  return (
    <div className="relative">
      ...
      {userBadge.enable_ring && (
        <div
          className="absolute -inset-[5px] rounded-xl animate-spin-slow"
          style={{
...
function Level67Effect({ user, children }: { user: any; children: ReactNode }) {
  ...
  return <div className="relative"><div className="absolute inset-0 rounded-xl animate-role-aura" style={{ boxShadow: `0 0 20px 4px ${color}40` }} />{children}</div>;
}
function Level8Effect({ user, children }: { user: any; children: ReactNode }) {
  const color = user.role?.color || "#f59e0b";
  return <div className="relative"><div className="absolute -inset-[4px] rounded-xl animate-spin-slow" style={{ background: `conic-gradient(from 0deg, ${color}, ${color}80, transparent, ${color}80, ${color})` }} /><div className="relative rounded-xl border-[3px] border-[#171717]">{children}</div></div>;
}
function Level9Effect({ children }: { children: ReactNode }) {
  return <div className="relative"><div className="absolute -inset-[4px] rounded-xl animate-spin-slow" style={{ background: "conic-gradient(from 0deg, #3b82f6, #60a5fa, transparent, #60a5fa, #3b82f6)" }} /><div className="relative rounded-xl border-[3px] border-[#171717]">{children}</div><span className="absolute -top-1 -right-1 text-[10px] font-mono text-blue-400 animate-float-1">&lt;/&gt;</span><span className="absolute -bottom-1 -left-1 text-[10px] font-mono text-blue-300 animate-float-2">{}</span></div>;
}
function Level10Effect({ children }: { children: ReactNode }) {
  return <div className="relative"><div className="absolute -inset-[5px] rounded-xl animate-spin-slow" style={{ background: "conic-gradient(from 0deg, #ffffff, #f3f4f6, #e5e7eb, #f3f4f6, #ffffff)" }} /><div className="relative">{children}</div><div className="absolute -top-4 left-1/2 -translate-x-1/2 w-9 h-9 animate-bounce-slow pointer-events-none select-none z-10"><img src="/hello-kitty.png" alt="" className="w-full h-full object-contain drop-shadow-[0_2px_8px_rgba(255,255,255,0.8)]" draggable={false} /></div></div>;
}
function Level11Effect({ children }: { children: ReactNode }) {
  return <div className="relative"><div className="absolute -inset-[4px] rounded-xl animate-spin-reverse" style={{ background: "conic-gradient(from 0deg, #10b981, #34d399, transparent, #34d399, #10b981)" }} /><div className="relative rounded-xl border-[3px] border-[#171717]">{children}</div><span className="absolute top-0 right-0 text-[8px] font-mono text-green-400 animate-float-1">01</span><span className="absolute bottom-0 left-0 text-[8px] font-mono text-green-300 animate-float-2">10</span></div>;
}