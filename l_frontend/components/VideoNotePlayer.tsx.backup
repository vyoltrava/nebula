...
export function VideoNotePlayer({ src, trackId, title }: { src: string; trackId?: string | number; title?: string }) {
  ...
  return (
    <button
      ...
      className={`relative rounded-2xl overflow-hidden bg-black ring-1 ring-white/10 active:scale-[0.98] transition-all duration-300 shrink-0 ${
        active && playing 
          ? "w-full max-w-[280px] aspect-square" // При проигрывании: занимает всю ширину бабла (до 280px), оставаясь квадратным
          : "w-52 h-52 sm:w-56 sm:h-56" // В покое: стандартный маленький размер
      }`}
    >
      <video
        ref={localRef}
