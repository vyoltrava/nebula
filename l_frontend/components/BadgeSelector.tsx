...
export function BadgeSelector({ currentUser, availableBadges, onUpdate }: { currentUser: any, availableBadges: any[], onUpdate: () => void }) {
  ...
  const hasBillet = currentUser?.billet_url;
    ...
    <div className="mt-4 p-4 rounded-xl bg-gray-100 dark:bg-white/5 border border-line dark:border-white/10">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <Sparkles size={16} className="text-purple-600 dark:text-purple-400" /> Значок аватара
        </h3>
        ...
        <div className="flex items-center gap-3">
          {hasBillet ? (
            <>
              ...
              >
                <img src={currentUser.billet_url} className="w-6 h-6 object-contain" alt="custom badge" />
              </div>
              <span className="text-sm text-gray-800 dark:text-white/70">Свой значок аватара</span>
            </>
          ) : currentBadge ? (
            <>
              <div 
            ...
            <span className="text-sm text-gray-500 dark:text-white/40">Значок аватара не выбран</span>
          )}
        </div>
      ) : (
        <div className="space-y-3">
          ...
          {canUploadCustom && (
            <div className="border-b border-line dark:border-white/10 pb-3">
              <p className="text-xs text-gray-600 dark:text-white/60 mb-2">Загрузить свой значок аватара:</p>
              <div className="flex gap-2">
                ...
                >
                  {uploading ? (
                    ...
                    <>
                      <Upload size={14} />
                      {hasBillet ? "Заменить" : "Загрузить"}
                    </>
                  )}
                </button>
                {hasBillet && (
                  <button 
                    onClick={deleteBillet}
