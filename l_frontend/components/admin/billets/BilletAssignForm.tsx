...
export function BilletAssignForm({ billet, billets, onClose, onSuccess }: AssignFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!selectedBillet) throw new Error("Пожалуйста, выберите плашку");
      ...
      // �?спользуем правильный эндпоинт из твоего main.py
            ...
            {searchResults.map((u) => (
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-400">@{u.username} Е ID: {u.id}</div>
                </div>
              </button>
            ))}
          </div>
