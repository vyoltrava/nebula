...
export function StatsSection({ me }: { me: any }) {
  ...
  if (!stats) return <p className="text-gray-600 dark:text-white/50 text-center py-12">Загрузка...</p>;
        ...
        <div className="border border-line dark:border-white/10 rounded-xl p-5 bg-gray-100 dark:bg-white/5">
          ...
          <div className="space-y-2">
            {stats.top_followers.map((u: any, i: number) => (
              <Link key={u.id} href={`/user/${u.id}`} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm text-gray-900 dark:text-white truncate">{u.display_name}</p>
