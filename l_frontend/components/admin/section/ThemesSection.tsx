...
/* ============================================================
   🎛️ Надёжный тумблер (switch).
   Габариты пилюли и ход ползунка заданы инлайново, а кнопка
   overflow-hidden — при любом состоянии ползунок физически
   не может выйти за край (фикс «выхода за границу» при вкл.).
   ============================================================ */
function ToggleSwitch({
  ...
  return (
    <button
      ...
      className={`relative shrink-0 overflow-hidden rounded-full transition-colors duration-200 disabled:opacity-50 ${
        checked
          ? tone === "emerald"
            ? "bg-emerald-500"
            : "bg-[#8b5cf6]"
    ...
    >
      <span
        ...
        className="absolute top-[2px] left-[2px] block h-5 w-5 rounded-full bg-white shadow transition-transform duration-200"
      />
    </button>
  );
}

/* Тема с расширением на будущее (уровень доступа) */
type EditableTheme = ThemeConfig & { min_level?: number };
...
export function ThemesSection({ me }: { me: any }) {
  ...
  /* 🎨 Конструктор тем (перенесён из /admin/themes) */
    ...
    <div className="space-y-4">
      ...
      <div className="border border-line dark:border-white/10 rounded-xl bg-gray-100 dark:bg-white/5 p-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <ToggleSwitch checked={shellEnabled} onChange={toggleShellSwitcher} disabled={saving} />
        </div>
      ...
      <div className="border border-line dark:border-white/10 rounded-xl bg-gray-100 dark:bg-white/5 p-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-gray-900 dark:text-white">Конструктор тем</h3>
          ...
          <button
            onClick={openCreate}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-purple-500 to-pink-500 text-white text-sm font-bold hover:shadow-lg hover:shadow-purple-500/30 transition-all shrink-0"
          >
            <Plus size={16} />
            Новая тема
          </button>
...
function ThemeEditor({
  ...
  theme: EditableTheme;
  onChange: (t: EditableTheme) => void;
  onSave: () => void;
  onClose: () => void;
}) {
  const ANIMATION_TYPES: { value: ThemeAnimationType; label: string; icon: typeof Palette; desc: string }[] = [
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="p-3 rounded-xl bg-gray-100 dark:bg-white/5 border border-line dark:border-white/10 flex items-center gap-3">
              ...
              <input
                type="number"
                min={0} max={10}
                value={theme.min_level ?? 0}
                onChange={(e) => onChange({ ...theme, min_level: Number(e.target.value) })}
