...
export default function AdminThemesPage() {
  ...
  // Загрузка тем СЃ Р±СЌРєРµРЅРґР° (если есть) + localStorage
  useEffect(() => {
    loadThemes();
    // Грузим состояние СЃ Р±СЌРєРµРЅРґР°
    const token = getToken();
    if (token) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-white/40 mt-0.5">
                  РЎРѕР·РґР°РІР°Р№ анимированные фоны Рґля всего СЃР°Р№та
                </p>
              </div>
            </div>
            <button
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-gray-900 dark:text-white">
                  Темы включены Рґля всех РїРѕР»СЊР·РѕРІР°С‚РµР»РµР№
                </p>
                <p className="text-[11px] text-gray-500 dark:text-white/40">
                  {globalEnabled ? "Анимированные фоны отображаются на СЃР°Р№те" : "Фон выключен Рґля всех вЂ” С‡РёСЃС‚С‹Р№ С‡С‘СЂРЅС‹Р№"}
                </p>
              </div>
            </div>
            <button
  ...
  return (
    <div className={`rounded-2xl overflow-hidden border transition-all ${
      ...
      <div className="p-3">
        ...
        >
          {theme.is_default ? "вњ“ По умолчанию" : "РЎРґелать по умолчанию"}
        </button>
      </div>
    </div>
  );
