...
if (user?.username === "trelod") return "#e4e4e7"; // Zinc-200
  ...
  async function deleteUser() {
    ...
    if (deleteConfirm !== selectedUser.username) {
      setSaveMsg({ text: "Введите username точно Рґля подтверждения", type: "err" });
      return;
    }
    setDeleting(true);
    setSaveMsg(null);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const data = await res.json();
        alert(`Аккаунт @${data.deleted_username} удалин. Постов: ${data.deleted_posts}`);
  ...
  async function clearLogs() {
    if (!confirm("РЈРґалить ВСЕ логи? Это действие только Рґля Founder.")) return;
    const token = getToken();
    if (!token) return;
    ...
    if (res.ok) {
      ...
      alert(`РЈРґалено записей: ${d.deleted}`);
      loadLogs();
    } else {
                          ...
                          !showReset2FAConfirm ? (
                            ...
                            <div className="space-y-2">
                              <p className="text-xs text-amber-300/90 leading-snug">
                                ⚠️ Пользователю придится заново настроить 2FA. Он сможет РІРѕР№ти по паролю.
