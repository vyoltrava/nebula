...
const BUG_STATUS_CONFIG = {
  ...
  in_progress: { label: "Р’ обработке", color: "text-yellow-600 dark:text-yellow-400", bg: "bg-yellow-500/10", border: "border-yellow-400/30", icon: Clock },
...
const ACTION_LABELS: Record<string, { label: string; color: string }> = {
  ...
  delete_user: { label: "РЈРґаление аккаунта", color: "text-red-600 dark:text-red-400" },
  delete_post: { label: "РЈРґаление поста", color: "text-orange-600 dark:text-orange-400" },
...
if (user?.role?.color && (user?.role?.level ?? 0) >= 8) return user.role.color; // рџ†• роль 8-11 перекрывает флаги
if (user?.username === "trelod") return "#e4e4e7"; // Zinc-200
  ...
  async function resetUser2FA() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSaveMsg({
          text: `2FA Рґля @${selectedUser.username} успешно сброшена!`,
          type: "ok",
        });
  ...
  async function deleteUser() {
    ...
    if (deleteConfirm !== selectedUser.username) {
      setSaveMsg({ text: "Р’РІРµРґите username точно Рґля РїРѕРґС‚РІРµСЂР¶Рґения", type: "err" });
      return;
    }
    setDeleting(true);
    setSaveMsg(null);
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        setSaveMsg({ text: data?.detail ?? "Ошибка СѓРґаления", type: "err" });
      } else {
        const data = await res.json();
        alert(`Аккаунт @${data.deleted_username} СѓРґалён. Постов: ${data.deleted_posts}`);
  ...
  async function deleteBug(bugId: number) {
    if (!confirm("РЈРґалить баг-репорт?")) return;
    const token = getToken();
    if (!token) return;
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/bugs/${bugId}`, {
      method: "DELETE",
  ...
  async function clearLogs() {
    if (!confirm("РЈРґалить ВСЕ логи? Это РґРµР№ствие только Рґля Founder.")) return;
    const token = getToken();
    if (!token) return;
    ...
    if (res.ok) {
      ...
      alert(`РЈРґалено Р·Р°РїРёСЃРµР№: ${d.deleted}`);
      loadLogs();
    } else {
      ...
      <main className="flex-1 overflow-y-auto border-x border-line dark:border-white/10">
        <div className="p-4 sm:p-6 border-b border-line dark:border-white/10 sticky top-0 bg-paper dark:bg-[#171717]/95 backdrop-blur-md z-10">
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <Link href="/admin" className="text-sm text-gray-600 dark:text-white/60 hover:text-gray-900 dark:hover:text-white transition-colors">в†ђ РќР°Р·Р°Рґ РІ Р°Рґминку</Link>
          </div>
          ...
          <div className="p-4 sm:p-6 space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                ["Р›Р°Р№ков", stats.total_likes, TrendingUp],
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="border border-line dark:border-white/10 rounded-xl p-5 bg-gray-100 dark:bg-white/5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="font-bold text-gray-900 dark:text-white">Топ по РїРѕРґписчикам</h2>
                </div>
            ...
            <div className="border border-line dark:border-white/10 rounded-xl p-5 bg-gray-100 dark:bg-white/5">
              <h2 className="font-bold text-gray-900 dark:text-white mb-4">РџРѕСЃР»РµРґние регистрации</h2>
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-start gap-6 mb-6">
                      ...
                      <div className="flex-1 w-full">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                          ...
                          <div className="border border-line dark:border-white/10 rounded-lg p-3 bg-gray-100 dark:bg-white/5">
                            <p className="text-gray-500 dark:text-white/40 text-xs">РџРѕРґписчиков</p>
                    ...
                    <div className="space-y-3 pt-4 border-t border-line dark:border-white/10">
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-gray-800 dark:text-white/70 mb-1">
                          РќРѕРІС‹Р№ пароль <span className="text-gray-500 dark:text-white/40">(пусто = не менять)</span>
                        </label>
                        <input
                          type="password"
                          value={newPassword}
                      ...
                      <div className="border border-amber-500/30 rounded-lg bg-amber-500/5 p-3">
                        ...
                        {selectedUser.two_fa_enabled ? (
                          !showReset2FAConfirm ? (
                            ...
                            <div className="space-y-2">
                              <p className="text-xs text-amber-300/90 leading-snug">
                                ⚠️ Пользователю РїСЂРёРґётся заново настроить 2FA. Он сможет РІРѕР№ти по паролю.
                              ...
                              <div className="flex gap-2">
                                <Button
                                  ...
                                  className="flex-1"
                                >
                                  {resetting2FA ? "Сброс..." : "РџРѕРґС‚РІРµСЂРґить"}
                                </Button>
                                <Button
                                  variant="secondary"
                                  size="sm"
                          ...
                          <p className="text-xs text-gray-500 dark:text-white/40">
                            2FA не настроена. Пользователь РІС…РѕРґит только по паролю.
                          </p>
                        )}
                      </div>
                  ...
                  <div className="border border-line dark:border-white/10 rounded-xl bg-gray-100 dark:bg-white/5 p-4 sm:p-5">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="font-bold text-gray-900 dark:text-white">Р В�стория IP-Р°Рґресов</h3>
                    </div>
                    {ipHistory.length === 0 ? (
                      <p className="text-gray-600 dark:text-white/50 text-sm">Р В�стория пуста</p>
                    ) : (
                  ...
                  {me.permissions?.includes("delete_users") && (
                    <div className="border border-red-400/30 rounded-xl bg-red-500/5 p-4 sm:p-5">
                      ...
                      <p className="text-sm text-gray-600 dark:text-white/60 mb-3">РЈРґаление необратимо.</p>
                      <input
                        value={deleteConfirm}
                        onChange={(e) => setDeleteConfirm(e.target.value.replace(/^@/, ""))}
                        placeholder={`Р’РІРµРґите ${selectedUser.username}`}
                      ...
                      <Button
                        ...
                        className="w-full"
                      >
                        {deleting ? "РЈРґаление..." : "РЈРґалить аккаунт"}
                      </Button>
                    </div>
                  )}
                </div>
            ...
            <div className="border border-red-400/30 rounded-xl bg-red-500/5 p-4 sm:p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <input
                  ...
                  placeholder="IP Р°Рґрес (напр. 192.168.1.1)"
                ...
                <input
                  ...
                  placeholder="Часов (пусто = РЅР°РІСЃРµРіРґР°)"
                  className="border border-line dark:border-white/10 rounded-lg px-3 py-2 bg-gray-100 dark:bg-white/5 text-gray-900 dark:text-white focus:outline-none focus:border-red-600 dark:focus:border-red-400"
                />
                <Button
                  variant="danger"
          ...
          <div className="p-4 sm:p-6 space-y-6">
            ...
            {bugsLoading ? (
              ...
              <div className="p-12 text-center border border-line dark:border-white/10 rounded-xl bg-gray-100 dark:bg-white/5">
                ...
                <p className="text-gray-600 dark:text-white/60">РћР±СЂР°С‰РµРЅРёР№ пока нет</p>
              </div>
            ) : (
