...
export default function AdminStickersPage() {
  ...
  // рџ†• Состояния Рґля нового СЂРµРґактора пака
  ...
  async function savePack() {
    ...
    if (!editingPack.name?.trim()) { alert("Р’РІРµРґРё название пака"); return; }
    const token = getToken();
    ...
    try {
      ...
      if (res.ok) {
        ...
        // рџ†• Если выбраны СЌРјРѕРґзи вЂ” Рґобавляем их РІ пак
  ...
  async function deletePack(id: number) {
    if (!confirm("РЈРґалить пак со всеми стикерами?")) return;
    const token = getToken();
  ...
  async function deleteSticker(stickerId: number) {
    if (!confirm("РЈРґалить стикер?")) return;
    const token = getToken();
    ...
    <div className="h-screen flex overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto border-x border-line dark:border-white/10">
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] sm:text-xs text-gray-500 dark:text-white/40">Реакции Рё стикеры Рґля чатов</p>
              </div>
            </div>
            <button
              onClick={() => {
        ...
        <div className="p-4 space-y-4 max-w-4xl mx-auto">
            ...
            <div key={pack.id} className={`border rounded-2xl p-4 ${pack.is_active ? "border-line dark:border-white/15 bg-gray-100 dark:bg-white/5" : "border-line dark:border-white/5 bg-white/[0.02] opacity-60"}`}>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                {pack.stickers.length === 0 && (
                  <p className="text-[11px] text-gray-500 dark:text-white/30">Пак пуст вЂ” Рґобавь стикеры ниже в†“</p>
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-3 border-t border-line dark:border-white/10">
                ...
                <div className="flex-1 flex gap-2">
                  <input
                    ...
                    placeholder="Вставь СЌРјРѕРґзи: рџ’Ђ рџ—ї рџ”Ґ (можно несколько через пробел)"
          ...
          {packs.length === 0 && (
            <p className="text-center text-gray-500 dark:text-white/40 py-16">Паков пока нет вЂ” СЃРѕР·РґР°Р№ РїРµСЂРІС‹Р№!</p>
          )}
        </div>

        {/* рџ†• РЈР›РЈР§РЁЕННЫЙ РЕДАКТОР ПАКА */}
            ...
            <div className="fixed inset-0 z-[301] flex items-center justify-center p-4 pointer-events-none overflow-y-auto">
              <div className="w-full max-w-2xl bg-ivory dark:bg-[#1f1f23] border border-line dark:border-white/15 rounded-2xl shadow-2xl p-5 pointer-events-auto my-8">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-black text-gray-900 dark:text-white text-lg">{editingPack.id ? "Р РµРґактировать пак" : "РќРѕРІС‹Р№ пак"}</h2>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Например: Мемы, Р’Р°Р№Р±, VIP..."
                  ...
                  {/* рџ†• ВЫБОР Р­Р СљР С›Р вЂќР вЂ”Р В� Р В�Р— Р вЂ�Р В�Р вЂ�Р вЂєР В�Р С›Р СћР вЂўР С™Р В� */}
                  <div className="border border-line dark:border-white/10 rounded-xl p-3 bg-white/[0.02]">
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-xs font-bold text-gray-600 dark:text-white/60">
                        Выбрать СЌРјРѕРґзи из библиотеки ({selectedEmojis.length} выбрано)
                      </label>
                    ...
                    <div className="grid grid-cols-8 sm:grid-cols-10 gap-1 max-h-48 overflow-y-auto p-1">
                      ...
                      {filteredStickers.length === 0 && (
                        <p className="col-span-full text-center text-gray-500 dark:text-white/30 text-xs py-4">Ничего не РЅР°Р№Рґено</p>
                      )}
                    </div>
                  </div>
                </div>
