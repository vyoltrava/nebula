...
export default function UpdatesPage() {
  ...
  async function createUpdate() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(d?.detail ?? t("updates.createError"));
        setSaving(false);
        return;
      }
      setTitle("");
  ...
  async function deleteUpdate(id: number) {
    if (!confirm(t("updates.deleteConfirm"))) return;
    const token = getToken();
    if (!token) return;
    ...
    <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#8b5cf6] text-white font-bold hover:bg-[#7c3aed] transition-all shadow-lg shadow-[#8b5cf6]/30">
      <Plus size={18} /> {t("updates.write")}
            ...
            {unreadCount > 0 && (
              ...
              >
                <CheckCheck size={16} />
                {t("updates.markAll", { n: unreadCount })}
              </button>
            )}
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-[201] flex items-center justify-center p-4 pointer-events-none">
            <div className="w-full max-w-xl border border-line dark:border-white/20 rounded-2xl bg-ivory dark:bg-[#1f1f23]/95 backdrop-blur-md shadow-2xl p-6 pointer-events-auto max-h-[85vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={() => setShowForm(false)} className="text-gray-600 dark:text-white/60 hover:text-gray-900 dark:hover:text-white p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/10">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-4">
                ...
                >
                  {saving ? t("updates.publishing") : t("updates.publish")}
                </button>
              </div>
            </div>
          </div>
