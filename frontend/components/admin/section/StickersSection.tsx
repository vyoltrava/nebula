...
export function StickersSection({ me }: { me: any }) {
  ...
  async function addEmoji(packId: number) {
    ...
    if (res.ok) { setEmojiInputs((prev) => ({ ...prev, [packId]: "" })); loadPacks(); }
  }
          ...
          <div className="flex flex-col sm:flex-row gap-2 pt-3 border-t border-white/10">
            ...
            <div className="flex-1 flex gap-2">
              <input value={emojiInputs[pack.id] || ""} onChange={(e) => setEmojiInputs((prev) => ({ ...prev, [pack.id]: e.target.value }))}
                onKeyDown={(e) => { if (e.key === "Enter") addEmoji(pack.id); }} placeholder="Вставь эмодзи: 💀 🗿 🔥"
          ...
          <div className="fixed inset-0 z-[301] flex items-center justify-center p-4 pointer-events-none overflow-y-auto">
            <div className="w-full max-w-2xl bg-[#1f1f23] border border-white/15 rounded-2xl shadow-2xl p-5 pointer-events-auto my-8">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input value={editingPack.name} onChange={(e) => setEditingPack({ ...editingPack, name: e.target.value })} placeholder="Например: Мемы, Вайб, VIP..."
                  ...
                  <div>
                    ...
                    <select value={editingPack.min_level} onChange={(e) => setEditingPack({ ...editingPack, min_level: Number(e.target.value) })}
                ...
                <div className="border border-white/10 rounded-xl p-3 bg-white/[0.02]">
                  ...
                  <div className="relative mb-2">
                    <input value={emojiSearch} onChange={(e) => setEmojiSearch(e.target.value)} placeholder="Поиск эмодзи..."
                      className="w-full pl-3 pr-3 py-2 rounded-lg border border-white/15 bg-white/5 text-white text-xs placeholder-white/30 focus:outline-none focus:border-yellow-400" />
                  </div>
                  {!emojiSearch && (
                    <div className="flex gap-1 mb-2 overflow-x-auto scrollbar-hide pb-1">
                      {CATEGORIES.map((cat) => (
                        <button key={cat.key} onClick={() => setActiveCategory(cat.key)}
                          className={`px-2.5 py-1 rounded-lg text-[10px] font-bold whitespace-nowrap shrink-0 ${activeCategory === cat.key ? "bg-yellow-500 text-black" : "bg-white/5 text-white/50 hover:bg-white/10"}`}>
                          {cat.label}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="grid grid-cols-8 sm:grid-cols-10 gap-1 max-h-48 overflow-y-auto p-1">
                    {filteredStickers.map((s) => {
                      ...
                      return (
                        <button key={s.code} onClick={() => toggleEmoji(s.emoji)} title={s.label}
                          className={`aspect-square flex items-center justify-center rounded-lg text-xl transition-all ${isSelected ? "bg-yellow-500/30 border-2 border-yellow-400 scale-110" : "bg-white/5 hover:bg-white/10 border border-transparent"}`}>
                          {s.emoji}
                        </button>
                      );
                    })}
              ...
              <div className="flex gap-2 mt-5">
                <button onClick={() => !savingPack && setShowEditor(false)} disabled={savingPack}
